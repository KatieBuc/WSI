...
def process_population_data(df: pd.DataFrame, iso_codes: list[str] | None = None) -> pd.DataFrame:
    ...
    age_group_mapping = {
        'SP.POP.2529.FE': '25-34', 'SP.POP.3034.FE': '25-34',
        'SP.POP.3539.FE': '35-44', 'SP.POP.4044.FE': '35-44',
        'SP.POP.4549.FE': '45-54', 'SP.POP.5054.FE': '45-54',
        'SP.POP.5559.FE': '55-64', 'SP.POP.6064.FE': '55-64',
    }
    ...
    return (
        df
        .groupby(['ISO_code', 'Year', 'Age Group'], as_index=False)
        ['Population']
        .sum()
    )


def process_employment_data(df: pd.DataFrame, iso_codes: list[str] | None = None) -> pd.DataFrame:
    # map ISO codes
...
def calculate_employment_metric(em_df: pd.DataFrame, pop_df: pd.DataFrame) -> pd.DataFrame:
